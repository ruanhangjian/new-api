...
- Key 选择器位于创建区标题行，使用“Key 名称-分组名称-脱敏 Key”展示；参数区固定为尺寸、质量、格式、透明背景和数量，模型选择保留在提交按钮左侧。
- 参数只根据 `/api/image-workshop/options` 返回的真实能力启用；审核选项不展示，透明背景在后端未声明支持时显示为不可操作的“关闭”，不向请求中伪造无效字段。
- `gpt-image-2` 的参数能力按模型家族声明，不再要求渠道地址必须是 `api.openai.com`。OpenAI 兼容中转同样开放 `auto/1024x1024/1536x1024/1024x1536` 尺寸、`auto/low/medium/high` 质量、`png/jpeg/webp` 格式和最多 4 张输出；提交时仍由 NewAPI relay 和上游执行真实校验。
