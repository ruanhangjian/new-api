...
- Phase 3A 分辨率档位与差异计费底座：已完成请求档位判定、固定价格模式倍率、实际输出尺寸审计字段和前端预计档位提示；管理员可配置价格表和多模型适配仍待后续拆分。
...
- 当前已补充分辨率计费底座：后端按实际发送的 `size` 推导预计上游档位，`auto` 或缺少尺寸时按 2K 保守处理；固定价格模式使用 1K=`1.0`、2K=`1.5`、4K=`2.0` 的默认倍率，token usage 模式不再叠加倍率，避免上游 usage 已体现分辨率时重复计费。
- 预览接口和任务记录会返回请求档位、倍率和落盘图片实际宽高。实际宽高只用于质量审计，不作为扣费依据；上游返回尺寸缺失或被降级时，仍按已发送请求档位结算。
...
- 固定价格模式默认按 1K=`1.0`、2K=`1.5`、4K=`2.0` 倍率结算；管理员后续应在模型价格中配置 1K 基准价格，并根据上游成本和站点毛利调整基准值。
- token usage 模式保留上游 usage 结算，不再额外叠加档位倍率，避免上游 usage 已反映分辨率时重复计费。
- 实际图片宽高、请求尺寸、预计档位和倍率全部保留在任务结果/日志中，只用于质量审计、上游适配纠错和异常告警。
- 1K 选项下某些非正方形预设的最大边可能已经超过 1024，因此前端同时展示“预计计费档位”；这类提示不改变已经验证过的生成尺寸行为。

后续增强按独立分支推进：

- 管理员可配置 1K、2K、4K 的实际对外价格或倍率，并在价格未配置时给出明确提示。
