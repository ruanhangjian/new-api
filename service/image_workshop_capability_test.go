...
func TestImageWorkshopCapabilityForModel(t *testing.T) {
	...
	highConcurrencyCapability, ok := imageWorkshopCapabilityForModel("gpt-image-2L", false)
	require.True(t, ok)
	assert.Equal(t, "gpt-image-2L", highConcurrencyCapability.Model)
	assert.Equal(t, capability.Sizes, highConcurrencyCapability.Sizes)
	assert.Equal(t, capability.SizeTiers, highConcurrencyCapability.SizeTiers)
	assert.Equal(t, capability.AspectRatios, highConcurrencyCapability.AspectRatios)
	assert.Equal(t, capability.Qualities, highConcurrencyCapability.Qualities)
	assert.Equal(t, capability.OutputFormats, highConcurrencyCapability.OutputFormats)
	assert.Equal(t, capability.MaxImages, highConcurrencyCapability.MaxImages)
	assert.Equal(t, capability.MaxReferenceImages, highConcurrencyCapability.MaxReferenceImages)
	assert.True(t, highConcurrencyCapability.SupportsCustomSize)
	assert.True(t, highConcurrencyCapability.SupportsReferenceImages)
	assert.True(t, highConcurrencyCapability.SupportsTransparentBackground)

	nanoBananaCapability, ok := imageWorkshopCapabilityForModel("Nano-Banana-2", false)
	require.True(t, ok)
	assert.Equal(t, "Nano-Banana-2", nanoBananaCapability.Model)
	assert.Equal(t, capability.Sizes, nanoBananaCapability.Sizes)
	assert.Equal(t, capability.SizeTiers, nanoBananaCapability.SizeTiers)
	assert.Equal(t, capability.AspectRatios, nanoBananaCapability.AspectRatios)
	assert.Equal(t, capability.Qualities, nanoBananaCapability.Qualities)
	assert.Equal(t, capability.OutputFormats, nanoBananaCapability.OutputFormats)
	assert.Equal(t, capability.MaxImages, nanoBananaCapability.MaxImages)
	assert.Equal(t, capability.MaxReferenceImages, nanoBananaCapability.MaxReferenceImages)
	assert.True(t, nanoBananaCapability.SupportsCustomSize)
	assert.True(t, nanoBananaCapability.SupportsReferenceImages)
	assert.True(t, nanoBananaCapability.SupportsTransparentBackground)
